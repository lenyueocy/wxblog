...
Page(extend({}, Tab, {
    ...
    handleChangeScroll ({detail}) {
        ...
        this.getCategoryData(detail.key);
    },
    getCategory: function () {
        const that = this
        api.getCategory({
            query: {
                urlCode: '/article/category'
            },
            success: (res) => {
                if (res.statusCode == 200) {
                    this.setData({categorys: res.data.data})
                    this.setData({current_scroll: res.data.data[0].menu_id})
                    that.getCategoryData(13)
                }
            }
        });
    },
    getCategoryData: function (id) {
        ...
        api.getCategoryArticle({
            query: {
                id: id,
                urlCode: '/article/categorydata'
            },
            success: (res) => {
                wx.showToast({
                    title: '加载完毕~',
